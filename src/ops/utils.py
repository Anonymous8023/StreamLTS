...
@torch.no_grad()
def points_in_boxes_gpu_2d(points, boxes, batch_size=None, batch_indices=None):
    cur_boxes = copy.deepcopy(boxes)
    cur_boxes[:, 3] = 0
    bev_pts = copy.deepcopy(points)
    bev_pts[:, 3] = 0
    _, box_idx_of_pts = points_in_boxes_gpu(bev_pts, cur_boxes,
